...
proc setupCredentialsAndTree() {.async.} =
  ...
  echo "Generating credentials for ", NodePeerIds.len, " nodes...\n"

  # Generate credentials for all nodes
  var allCredentials: seq[tuple[peerId: string, cred: IdentityCredential]]
  for peerId in NodePeerIds:
    let cred = generateCredentials().valueOr:
      echo "Failed to generate credentials for ", peerId, ": ", error
      quit(1)

    allCredentials.add((peerId: peerId, cred: cred))
  ...
  # Register all credentials in the tree
  echo "Registering all credentials in the Merkle tree..."
  for i, entry in allCredentials:
    let index = (await groupManager.register(entry.cred.idCommitment)).valueOr:
      echo "Failed to register credential for ", entry.peerId, ": ", error
      quit(1)
    echo "  Registered ", entry.peerId, " at index ", index
  ...
  echo ""
  echo "=== Setup Complete ==="
  echo "  Tree file: rln_tree.db (", NodePeerIds.len, " members)"
